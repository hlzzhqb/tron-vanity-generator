...
`--backend gpu` / `auto` 选中 GPU 后会长时间接近满载占用显卡。在集成显卡（尤其是同一块芯片
既算图形又管显示）上，单次 GPU kernel 调度如果连续跑超过 1 秒且中间完全不让出，个别机器 /
驱动组合下被观察到会导致显示子系统（DWM）卡死、需要强制重启，且不一定能被 Windows 自动的
TDR 恢复机制救回来。v1.0.1 起把单次 kernel 覆盖的私钥数从 2^20 降到 2^16（约 1.3s → 约
80ms/次），大幅降低了这个风险，但**不保证在所有硬件上都不会再发生**。

- 长时间无人值守（比如过夜挂机）建议先用 `--backend cpu`，确认没问题后再考虑 GPU。
- 如果画面黑屏且按 `Ctrl+Shift+Win+B`（重启显卡驱动的系统快捷键）没反应、键盘 CapsLock/
  NumLock 指示灯还能切换，说明是显示子系统卡死而非整机死机，大概率和 GPU 负载有关。
- 升级显卡驱动到最新版本也有帮助。
