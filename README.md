...
- `>= N` 位**相同字符**，如 `...AAAAA`
- `>= N` 位**连续字符**：按字符 ASCII 值 ±1 递增或递减，且整段落在**同一类别**内
  （全数字 / 全小写 / 全大写）。如 `...12345`、`...abcde`、`...WXYZ`、`...54321`、`...edcba`
  - **不算**连续：`89123`（跨过 base58 里没有的 `0`）、`xyzab`（`z→a` 回绕）、
    `9abcd`（数字跨到字母）、`FGHJ`（跳过 base58 里没有的 `I`）
